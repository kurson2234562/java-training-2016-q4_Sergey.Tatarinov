...
public class Query {
    ...
    public static final String SELECT_STUDENT_WITHOUT_MARK_BY_COURSE = "SELECT surname, name, patronymic FROM STUDENT_COURSE INNER JOIN STUDENTS ON STUDENTS.ID=STUDENT_COURSE.ID_STUDENT WHERE STUDENT_COURSE.ID_STUDENT_COURSE=?";
    ...
    public static final String ALL = "select /*STUDENTS.ID,*/ STUDENTS.SURNAME, STUDENTS.NAME /*STUDENTS.PATRONYMIC*/, NAME_COURSE, DURATION, NAME_STATUS, THEMES.name_theme, LECTURERS.surname, LECTURERS.name, LECTURERS.patronymic from students inner join student_course on student_course.id_student=students.id inner join courses on student_course.id_course=courses.id_course inner join statuses on statuses.id_status = courses.id_status inner join themes on COURSES.id_theme = THEMES.id_theme inner join LECTURERS ON COURSES.id_lecturer = LECTURERS.id;";
}
